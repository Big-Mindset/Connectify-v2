...
const ChatSettings = dynamic(()=>import("./chat-settings"))

export default function ChatUser({ chat , setChatSettings , isMenuOpen }) {
    ...
    const handleGetChat = async () => {
        ...
        await getChatById(chatInfo)

    }
    let status = messageStatus(chat?.lastMessage?.status)
    let userData = participants.get(chat.userId)
    ...
    let typingUsers = useMemo(() => {
        ...
        if (typingIndicators) {
            ...
            for (let name of [...typingIndicators].slice(0, 3)) {
   ...
   useEffect(() => {
    ...
    return (
        ...
        <div onClick={handleGetChat} className={` shrink-0 relative  overflow-hidden ${selectedChat?.id === chat?.id ? "ring-2 ring-indigo-500 bg-gray-600/10" : "ring hover:ring-indigo-500"}  flex px-2.5   group cursor-pointer duration-100 ring-gray-2 py-2 bg-[#00000094] rounded-lg items-center gap-2`}>
            ...
            <div onClick={handleOpenSettings} className="bg-gray-800/30 backdrop-blur-2xl invisible group-hover:visible   absolute -right-5 top-0 p-1.5 duration-100 group-hover:right-0 rounded-full  ">
                    ...
                    : chat.lastMessage ?
                        <div className="flex items-center  justify-between">
                            <div className="flex items-center  min-w-[0] flex-1 gap-0.5">
                                ...
                                <p className="text-[0.8rem] truncate ml-0.5 text-gray-300/80   min-w-[0] ">{chat.lastMessage.content}</p>
                                {!chat.lastMessage.content &&
                                    <span className="text-gray-400 text-[0.8rem]">{chat?.lastMessage?.media[0]?.type}</span>
                                }
                            </div>
                        ...
                        : <p className="text-sm text-gray-400 font-bold  ">{chat?.description || userData?.username}</p>
                    }
            </div>
